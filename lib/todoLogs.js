...
class TodoLog {
  ...
  searchMatchingTodo(title) {
    return this.todoLogs.filter(
      todo => todo.title.includes(title) && title !== ''
    );
  }

  searchMatchingTask(task) {
    const matchingTask = this.todoLogs.filter(todo =>
      todo.hasMatchingTask(task)
    );
    return matchingTask;
  }
